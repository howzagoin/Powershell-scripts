...
foreach ($mailbox in $mailboxes) {
    ...
    try {
        ...
        # Check for redirect rules to external email addresses (not internal contacts)
        $redirectRules = $rules | Where-Object {
            $_.RedirectTo -and $_.RedirectTo -match "^[\w.%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$" -and $_.RedirectTo -notlike "*@$(($mailbox.PrimarySmtpAddress -split '@')[1])"
        }

        # Display if any valid external redirect rules are found
