...
#$TempLocation = C:\Program Files\ATERA Networks\AteraAgent\Packages\AgentPackageSystemTools

# Define Wiztree file paths
$sourcePath = "https://www.diskanalyzer.com/files/wiztree_4_20_portable.zip"  # Replace with the actual path to the downloaded WizTree portable zip file
...
#old line $CSVContent = Get-Content -Path $TempLocation\$FilesCSV -ReadCount 0 
$CSVContent = $csvrow -split'([^”,]+|”[^”]*”)’ -notmatch ‘^,?$’
...
# Define a CSS style for the HTML report
$style = @"
<style>
...
# Export the largest 100 results into an HTML format
$Table |
    Sort 'Size (Bytes)' -Descending |
    Select -First 100 |
